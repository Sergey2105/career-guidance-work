...
export const addField = createAsyncThunk("voting/addVoting", async ({ id, name }: { id: string; name: any }, thunkAPI) => {
    const token = localStorage.getItem("userToken");
    if (token !== null) {
...
export const destroyField = createAsyncThunk("voting/addVoting", async ({ id }: { id: string }, thunkAPI) => {
    const token = localStorage.getItem("userToken");
    if (token !== null) {
...
export const addVote = createAsyncThunk("event/joinEvent", async ({ id }: { id: string }, thunkAPI) => {
    const token = localStorage.getItem("userToken");
    if (token !== null) {
